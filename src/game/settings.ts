...
export const SETTING_RANGES = {
  ...
  // to taste. Widened from the old 0.8-1.3 band to 0-200% on live feedback: 0
  // hides the buttons (recoverable, since the settings menu itself is NOT scaled
  // by --btn-scale), 100% is stock, 200% is the large end. def stays 1.0x so no
  // stored value migrates (the load clamp only widens the interval). Surfaced in
  // the Esc menu only on phone-touch devices.
  actionButtonScale: { min: 0, max: 2, def: 1 },
