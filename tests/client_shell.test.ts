...
describe('client HTML shell', () => {
  ...
  it('seats Autorun by the joystick and keeps Jump bound from the ring markup', () => {
    for (const [name, entry] of [
      ['index.html', html],
      ['play.html', playHtml],
    ] as const) {
      ...
      // Jump stays in the RING markup and binding path, while portrait CSS can
      // move its visible seat into the open lower strip. Use stays in the ring
      // hollow. Neither may reappear in the left cluster.
